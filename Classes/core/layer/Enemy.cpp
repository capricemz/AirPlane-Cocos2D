...
bool Enemy::init(TypeEnemy value, CallFuncN *actionRemove4Vec)
{
	...
	do 
	{
		...
		auto xActual = UtilRandom::randomBewteen(sizeEnemy.width/2,sizeWin.width - sizeEnemy.width/2);//随机初始位置
...
void Enemy::blowup()
{
	ManagetLayer::getInstance()->layerUIGet()->updateScore(_score);//添加分数
	//
	auto animation = Animation::create();
	switch (_type)
	{
